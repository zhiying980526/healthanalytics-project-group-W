...
model2 <- lm(BIRTHAG ~ EDUCREC2 + INCFAM97ON2 + MARSTAT, data = filtered_data)
summary(model2)
...
summary(model3)
# 按种族分组运行回归 有问题RACEA数据信息不符合要求 后面的race_models income_models都有问题
race_models <- filtered_data %>%
  group_by(RACEA) %>%
  group_modify(~ {
    model <- lm(BIRTHAG ~ EDUCREC2 + INCFAM97ON2 + MARSTAT, data = .x)
...
income_models <- filtered_data %>%
  group_by(INCFAM97ON2) %>%
  group_modify(~ {
    model <- lm(BIRTHAG ~ EDUCREC2 + MARSTAT, data = .x)
    return(broom::tidy(model))
  })
# 修改region-------------------------------REGION缺少数据
# 创建 REGION 分组
...
ggplot(income_models, aes(x = term, y = estimate, color = INCOME_GROUP)) +
  geom_point(size = 3) +
